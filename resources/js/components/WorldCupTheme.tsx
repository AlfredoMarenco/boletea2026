...
function makeParticle(
    ...
    const type: 'confetti' | 'ball' =
        Math.random() > 0.82 ? 'ball' : 'confetti';
    return {
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            if (cursor) {
                ...
                posRef.current.x +=
                    (targetRef.current.x - posRef.current.x) * 0.14;
                posRef.current.y +=
                    (targetRef.current.y - posRef.current.y) * 0.14;
...
export default function WorldCupTheme() {
    ...
    useEffect(() => {
        ...
        return () =>
            window.removeEventListener('world-cup:trigger-goal', handleTrigger);
    }, []);
            ...
            .then((res) => {
                ...
                if (data.enabled) {
                    const timeDiff = Math.abs(
                        Math.floor(Date.now() / 1000) - data.last_goal_time,
                    );

                    if (
                        prevLastGoalTimeRef.current !== null &&
                        data.last_goal_time > prevLastGoalTimeRef.current
                    ) {
                        triggerGoalCelebration();
                    } else if (
                        prevLastGoalTimeRef.current === null &&
                        data.last_goal_time > 0 &&
                        timeDiff < 30
                    ) {
                        triggerGoalCelebration();
                    }
    ...
    useEffect(() => {
        ...
        const update = () => {
            ...
            if (targetStr) {
                ...
                if (
                    !targetStr.includes('Z') &&
                    !targetStr.includes('+') &&
                    !/[-+]\d{2}:?\d{2}$/.test(targetStr)
                ) {
        ...
        for (let i = 0; i < 220; i++) {
            ...
            bursts.push(
                makeParticle(
                    ...
                    h,
                    Math.cos(aL) * spL,
                    -Math.abs(Math.sin(aL) * spL),
                ),
            );
            // Right corner
            const aR =
                Math.PI - Math.random() * (Math.PI * 0.55) - Math.PI * 0.06;
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            for (let i = pts.length - 1; i >= 0; i--) {
                ...
                // Cleanup
                if (
                    (p.isExplosion && p.life <= 0) ||
                    (!p.isExplosion && p.y > canvas.height + 60)
                ) {
                    pts.splice(i, 1);
                }
            }
    ...
    return (
        <>
            ...
            {/* Top banner */}
            {!isAdminPage && (
                <div className="fixed top-0 right-0 left-0 z-[60] flex h-9 items-center justify-center overflow-hidden border-b border-black/10 bg-gradient-to-r from-[#006847] via-[#f7f7f7] to-[#CE1126] text-[11px] font-bold shadow-md select-none sm:text-xs">
                    <div className="absolute inset-x-0 bottom-0 h-[2px] bg-yellow-400 opacity-80" />

                    {status.status === 'countdown' && (
                        <div className="flex animate-pulse items-center gap-1 text-gray-900 sm:gap-1.5">
                            <FlagImg country="México" height={12} />
                            <span className="hidden font-black sm:inline">
                                México
                            </span>
                            ...
                            <span className="rounded bg-black px-1.5 py-0.5 font-mono text-[10px] text-white shadow-sm sm:text-[11px]">
                    ...
                    {status.status === 'live' && (
                        <div className="flex items-center gap-1 text-gray-900 sm:gap-1.5">
                            <span className="inline-flex shrink-0 animate-pulse items-center rounded bg-red-600 px-1 py-0.5 text-[8px] font-black tracking-wider text-white uppercase shadow-sm sm:text-[9px]">
                                🔴 EN VIVO
                            </span>
                            ...
                            <span className="font-black sm:hidden">MEX</span>
                            <span className="rounded-full bg-black px-2 py-0.5 text-xs font-black text-white shadow-inner sm:text-sm">
                                {status.mexico_score}
                            </span>
                            <span className="font-bold">-</span>
                            <span className="rounded-full bg-black/80 px-2 py-0.5 text-xs font-black text-white shadow-inner sm:text-sm">
                                {status.opponent_score}
                            </span>
                            ...
                            <span className="hidden font-normal text-gray-500 md:inline">
                                |
                            </span>
                            <span className="hidden animate-bounce font-black text-[#006847] md:inline">
                                ¡VAMOS MÉXICO! ⚽🔥
                            </span>
                        </div>
                    )}
                    ...
                    {status.status === 'finished' && (
                        <div className="flex items-center gap-1 text-gray-900 sm:gap-1.5">
                            <span className="inline-flex shrink-0 items-center rounded bg-gray-600 px-1 py-0.5 text-[8px] font-black tracking-wider text-white uppercase shadow-sm sm:text-[9px]">
                                FINALIZADO
                            </span>
                            <span className="hidden sm:inline">
                                Marcador Final:
                            ...
                            <span className="text-xs font-extrabold sm:text-sm">
                                {status.mexico_score} - {status.opponent_score}
                            </span>
                            <FlagImg country={status.opponent} height={13} />
                            <span className="hidden font-black sm:inline">
                                {status.opponent}
                            ...
                            <span className="hidden text-[#006847] md:inline">
                                ¡Gracias por vivir el fútbol en Boletea! 🎉
                            </span>
                        </div>
                    )}
                </div>
            ...
            {isGoalActive && (
                <div
                    className="pointer-events-none fixed inset-0 z-[100] flex flex-col items-center justify-center overflow-hidden bg-black/75 backdrop-blur-[4px] select-none"
                    style={{ animation: 'wcFadeIn 0.3s ease-out forwards' }}
                >
                    <div
                        className="relative flex w-full items-center overflow-hidden border-y-2 border-yellow-400 bg-gradient-to-r from-[#006847] via-[#CE1126] to-[#006847] py-3 shadow-2xl sm:border-y-4 sm:py-4"
                        style={{ transform: 'rotate(-3deg) scaleX(1.08)' }}
                    >
                        ...
                        <div
                            className="flex items-center gap-8 text-3xl font-black tracking-wider whitespace-nowrap text-white uppercase sm:gap-12 sm:text-5xl md:text-6xl lg:text-7xl"
                            style={{
                                animation: 'wcScrollGol 8s linear infinite',
                            }}
                        >
                    ...
                    <div className="mt-4 flex flex-col items-center gap-1.5 sm:mt-6">
                        <div className="flex items-center gap-2 rounded-full border border-yellow-400 bg-black/60 px-5 py-1.5 text-base font-black text-white shadow-lg sm:gap-3 sm:border-2 sm:px-6 sm:py-2 sm:text-2xl">
                            <span>MÉXICO ⚽</span>
                            <span className="text-lg font-black text-yellow-400 sm:text-3xl">
                                {status.mexico_score}
                            </span>
                        </div>
                        <div className="mt-0.5 animate-pulse font-mono text-[8px] tracking-widest text-white/80 uppercase sm:mt-1 sm:text-[9px]">
                            ⚽ TRANSMISIÓN EN VIVO ⚽
                        </div>
                    </div>
                </div>
