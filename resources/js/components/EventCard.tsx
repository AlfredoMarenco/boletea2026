...
export default function EventCard({ event }: { event: ExternalEvent }) {
    ...
    return (
        <div className="group relative flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl dark:bg-card border border-gray-100 dark:border-border">
            <div className="relative w-full aspect-[5/4] bg-gray-200 dark:bg-card overflow-hidden">
                {event.image_path ? (
                ...
                {/* Date Badge */}
                {dateObj && (
                    <div className="absolute top-4 right-4 flex flex-col items-center justify-center bg-white/95 backdrop-blur-md rounded-xl p-2 min-w-[60px] shadow-lg border border-gray-100 dark:border-gray-100 dark:bg-white/80">
                        <span className="text-xs font-bold text-gray-500 dark:text-black uppercase tracking-widest">{month}</span>
